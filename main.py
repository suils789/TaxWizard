...
def on_extract_w2(btn):
    ...
    try:
        result = gpt_json  # gpt-4o 返回的就是 dict，不需要解析

        # 根据w2提取的信息自动填入身份字段，不需要手动填写
        ui.ssn.value = result.get("ssn", "")
        ui.first_name.value = result.get("first_name", "")
        ui.last_name.value = result.get("last_name", "")
...
def on_submit(btn):
    ...
    merged_data = {
        ...
        "exempt_as_teacher_or_trainee_yes": "Yes" if ui.exempt_as_teacher_or_trainee.value == "Yes" else "",
        "exempt_as_teacher_or_trainee_no": "Yes" if ui.exempt_as_teacher_or_trainee.value == "No" else "",
        ...
        "green_card_explanation_1": ui.green_card_explanation.value.split("\n")[0] if ui.green_card_explanation.value else "",
        "green_card_explanation_2": ui.green_card_explanation.value.split("\n")[1] if len(ui.green_card_explanation.value.split("\n")) > 1 else "",
        "green_card_explanation_3": ui.green_card_explanation.value.split("\n")[2] if len(ui.green_card_explanation.value.split("\n")) > 2 else "",
